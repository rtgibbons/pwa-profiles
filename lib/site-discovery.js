...
// Never automatically follow a redirect: even a rejected response may have
// already contacted its target. Opaque redirect responses must fail closed.
export async function fetchSameOrigin(url, selectedOrigin = new URL(url).origin, fetcher = fetch) {
  let current = new URL(url);
  for (let redirects = 0; redirects <= 5; redirects += 1) {
    if (current.origin !== selectedOrigin || current.username || current.password) {
      throw new Error("Cross-origin discovery is blocked. Enter the final site URL directly.");
    }
    const response = await fetcher(current.href, { redirect: "manual", credentials: "omit", referrerPolicy: "no-referrer" });
    if (response.type === "opaqueredirect") {
      throw new Error("Chrome hid this redirect. Enter the final site URL directly.");
    }
    if ([301, 302, 303, 307, 308].includes(response.status)) {
      const location = response.headers.get("location");
      if (!location) throw new Error("Redirect has no readable destination.");
      current = new URL(location, current);
      continue;
    }
    if (response.url && new URL(response.url).origin !== selectedOrigin) throw new Error("Cross-origin response rejected.");
    if (!response.ok) throw new Error(`Website returned HTTP ${response.status}.`);
    return { response, url: response.url || current.href };
  }
  throw new Error("Too many redirects (maximum five).");
}
